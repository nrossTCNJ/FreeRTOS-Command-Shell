...
A bare-metal embedded systems project built on the STM32F411RE (NUCLEO board) that integrates multiple hardware peripherals under a FreeRTOS preemptive scheduler. The system exposes a UART command shell, streams IMU sensor data, controls PWM-driven LED brightness, and performs RFID-based access control — all running concurrently as independent RTOS tasks.

This is the fifth and final project in a self-directed embedded systems curriculum, building on four prior projects: a bare-metal UART command shell, an I2C IMU dashboard, a PWM/timer controller, and an SPI RFID reader.
...
| `mfrc522_task` | Above Normal | `vTaskDelay` | RFID card detection, UID-based auth |
...
Two mutexes protect shared hardware:
...
- **`pwm_mutex`:** Guards `pwm_val` and `TIM3->CCR1` shared between `pulse_task` and `uart_task` (`dim`, `pulse`, `stop_pulse` commands).
...
This pattern ensures tasks block with zero CPU consumption between events, and `portYIELD_FROM_ISR` triggers an immediate context switch if a higher-priority task was unblocked.
...
- ACK management — re-enabled after single-byte reads to prevent transaction corruption
...
- JTAG disabled via `SYSCFG->MEMRMP` to free PB3 (SWO pin) for SPI SCK
...
- OC1PE preload enabled with UG/EGR update sequence to prevent CCR glitches
...
The detection state machine prevents duplicate scans:
...
**JTAG/SPI conflict on PB3:** PB3 is shared with SWO (JTAG trace). After reset, JTAG holds the pin. Resolved by disabling JTAG while preserving SWD via `SYSCFG->MEMRMP`.
...
└── freertos.c          # FreeRTOS CMSIS_V2 entry point
```
...
| Project | Focus |
|---------|-------|
| [Bare-Metal UART Shell](https://github.com/) | Interrupt-driven RX, ring buffer, command dispatcher |
| [I2C Sensor Dashboard](https://github.com/) | Bare-metal I2C1, MPU-6500 burst read, non-blocking streaming |
| [PWM Timer Control](https://github.com/) | TIM3 PWM, TIM4 interrupt-driven pulse state machine |
| [SPI RFID Reader](https://github.com/) | MFRC522 driver, UID-based access control, clone chip handling |
